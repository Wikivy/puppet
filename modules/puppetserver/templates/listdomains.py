...
EXEMPT_DOMAINS = ["analytics.wikivy.com", "grafana.wikivy.com", "monitoring.wikivy.com", "phorge-static.wikivy.com",
    "static.wikivy.com", "wikivy.com"]
PROXY = "http://bastion.wikivy.wvnet:8080"
...
def get_wikidiscover_data():
    url = 'https://meta.wikivy.com/w/api.php'
...
def git_push(workdir):
    ...
    subprocess.run([
        ...
        "ssh -i /var/lib/nagios/id_ed25519 -F /dev/null -o ProxyCommand='nc -X connect -x bastion.wikivy.wvnet:8080 %h %p'"
    ], check=True)

    subprocess.run(["git", "-C", workdir, "config", "user.name", "WikivyBot"], check=True)
    subprocess.run(["git", "-C", workdir, "config", "user.email", "noreply@wikivy.com"], check=True)
