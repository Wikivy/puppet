...
define systemdlib::unit (
  ...
  if ($source == undef) == ($content == undef) {
    fail("systemdlib::unit: ${title}: either source or content must be provided, but not both")
  }

  if ($unit =~ /^(.+)\.(\w+)$/ and $2 =~ Systemdlib::Unit::Type) {
    $unit_name = $unit
  } else {
