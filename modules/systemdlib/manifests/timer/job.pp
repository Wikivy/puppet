...
define systemdlib::timer::job (
  Variant[
    Systemdlib::Timer::Schedule,
    Array[Systemdlib::Timer::Schedule, 1]] $interval,
  String                                  $description,
  Systemdlib::Command                     $command,
  String                                  $user,
  ...
  Optional[Systemdlib::Input]                $stdin                     = undef,
  Optional[Systemdlib::Output]               $stdout                    = undef,
  Optional[Systemdlib::Output]               $stderr                    = undef,
  Optional[Stdlib::Unixpath]              $working_directory         = undef,
  Optional[Systemdlib::Command]              $exec_start_pre            = undef,
  ...
  $input_intervals = $interval ? {
    Systemdlib::Timer::Schedule => [$interval],
    default                  => $interval,
  }
  ...
  systemdlib::unit { "${title}.service":
    ensure  => $ensure,
    content => template('systemd/timer_service.erb'),
  }

  systemdlib::timer { $title:
    ensure          => $ensure,
  ...
  if $logging_enabled {
    ...
    systemdlib::syslog { $syslog_title:
      ensure                 => $ensure,
