...
# === Parameters
# [*timer_intervals*]
#   Array of Systemdlib::Timer::Schedule intervals at which the unit will be
...
#   Systemdlib::Timer::Interval. Defaults to 15 seconds, which should be good in
#   most cases.

define systemdlib::timer (
  Array[Systemdlib::Timer::Schedule, 1] $timer_intervals,
  String $unit_name="${title}.service",
  VMlib::Ensure $ensure = 'present',
  Integer $splay = 0,
  Systemdlib::Timer::Interval $accuracy = '15sec',
) {
  if $ensure == 'present' {
    $timer_intervals.each |$schedule| {
      ...
      if $interval !~ Systemdlib::Timer::Interval {
  ...
  # Timer service
  systemdlib::service { $title:
    ensure    => $ensure,
    unit_type => 'timer',
    content   => template('systemd/systemd.timer.erb'),
    require   => Systemdlib::Unit[$unit_name],
  }
}