...
class _WikivyRewriteContext(WSGIContext):
    """
        ...
        try:
            ...
            if match:
                if match.group('proj').endswith('wikibeta'):
                    ...
                    hostname = '%s.wikivy.dev' % (proj)
                else:
                    proj = match.group('proj').removesuffix("wiki")
                    hostname = '%s.wikivy.com' % (proj)
	...
	# Containers have 1 component: container.
       ...
       # Phonos renderings
        if match is None:
            match = re.match(r'^/(?P<proj>[^/]+)/(?P<repo>phonos-render)/(?P<path>.+)$', req.path)
            if match:
                proj = match.group('proj')
                repo = 'phonos'
                zone = 'render'
        ...
        if match:
            ...
            if zone:
                 container = "wikivy-%s-%s-%s" % (proj, repo, zone)
            else:
                 container = "wikivy-%s-%s" % (proj, repo)
...
class WikivyRewrite(object):
    ...
    def __call__(self, env, start_response):
        ...
        context = _WikivyRewriteContext(self, self.conf)
...
def filter_factory(global_conf, **local_conf):
    ...
    def wikivyrewrite_filter(app):
        return WikivyRewrite(app, conf)

    return wikivyrewrite_filter
