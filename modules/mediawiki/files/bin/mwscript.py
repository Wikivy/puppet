...
def get_commands(args: argparse.Namespace) -> CommandInfo | int:
    ...
    if wiki == 'all':
        long = True
        command = f'sudo -u {args.runas} /usr/local/bin/foreachwikiindblist /srv/mediawiki/cache/databases.php {script}'
    elif wiki and wiki in validDBLists:
        long = True
        command = f'sudo -u {args.runas} /usr/local/bin/foreachwikiindblist /srv/mediawiki/cache/{wiki}.php {script}'
    elif args.extension:
        long = True
        generate = f'sudo -u {args.runas} php {runner}WikivyMagic:GenerateExtensionDatabaseList --wiki=meta{wikisuffix} --extension={args.extension} --directory=/tmp'
        command = f'sudo -u {args.runas} /usr/local/bin/foreachwikiindblist /tmp/{args.extension}.php {script}'
    else:
        command = f'sudo -u {args.runas} php {script} --wiki={wiki}'
    if args.arguments:
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--runas', default='www-data')
